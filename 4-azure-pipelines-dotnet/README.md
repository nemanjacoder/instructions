...
- Set up Azure Pipelines to build and test a .NET Core project hosted in GitHub.
...
## 1. Fork the .NET Core project

To demonstrate a continous integration build and test setup, we'll use a .NET Core application with xunit unit tests.  We'll make a copy of this project, or a "fork", into our own GitHub repository so that we can work on it freely, without impacting the original open source  project.

* Open your browser and navigate to [https://github.com/azure-devops-labs/dotnet](https://github.com/azure-devops-labs/dotnet).

* If you haven't already, sign in to GitHub.

  ![sign in](images/1-signin.png)

  When you've signed in, you'll be returned to the dotnet repository.
...
* Set your Project name to "dotnet".
...
* Set the project name to "dotnet".  Set the visibility to "public".  Then click "Create".
...
* Select the repository that you want to build.  This will be the `dotnet` repository that you forked in step 1.  (It should be at the top of the repository list.)

  ![Select repository](images/5-selectrepo.png)

* Azure Pipelines will now analyze your repository to determine what language is it, and how it should build it.  Once it's finished with the examination, it will present you with some choices.  Select ".NET Desktop" from the options.
...
* Now, Azure Pipelines will show you the build definition.  Build definitions are written in the YAML language and checked in to the repository, right next to the code they build.  This is a technique called "configuration as code" and is helpful since it versions the steps to build the code alongside the code itself.
  ...
  Next, the pool is set up.  This build will run on the `windows-latest` pool, which indicates that it will run on Azure Pipelines' cloud-hosted Windows build agents.

  Then the build steps are defined: the default steps only restore the dependencies with nuget, then perform a build, and then run the test suite.  We want to update these steps to build in the latest version of .NET Core.
...
* Scroll to line **64**.  Update the line to remove the two-character formatting.  The line should look like:

  ```
    return String.Format("{x2}", b);
  ```

  This changes the formatting so that hex bytes are no-longer zero-padded to two charactersss.  (As a result, the unit tests will fail).
...
* Scroll down to the bottom of the page to commit this change.  In the first text field, the commit title, enter "Update hex formatting".
...
Now you've set up a continuous integration build for your .NET Core project on GitHub, and you've seen how to customize the build and how it operates on GitHub Pull Requests.
